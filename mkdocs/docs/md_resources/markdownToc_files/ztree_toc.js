...
*/
    ...
    function bind_scroll_event_and_update_postion(b) {
        var c, d = function () {
            c && clearTimeout(c), c = setTimeout(function () {
                var c = $(window).scrollTop();
                b.debug && console.log("top=" + c);
                for (var d = 0, e = b._header_offsets.length; e > d; d++)if (b._header_offsets[d] >= c + 5) {
                    console.log("opts._header_offsets[" + d + "] = " + b._header_offsets[d]), $("a").removeClass("curSelectedNode"), $("#tree_" + (d + 1) + "_a").addClass("curSelectedNode");
                    break
                }
            }, b.refresh_scroll_time)
        };
        b.highlight_on_scroll && ($(window).bind("scroll", d), d())
    }
	...
	$.fn.ztree_toc.defaults = {
		...
		_headers: [],
		_header_offsets: [],
		_header_nodes: [{ id:1, pId:0, name:"帮助手册目录索引",open:true}],
		debug: true,
		/*
		 * 使用标题作为anchor
		 * create table with head for anchor for example: <h2 id="#Linux基础">Linux基础</h2>
